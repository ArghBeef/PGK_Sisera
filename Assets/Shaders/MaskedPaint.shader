Shader "Custom/MaskedPaint"
{
    Properties
    {
        ...
        _EmissionMask ("Emission / Paint Mask", 2D) = "black" {}
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            Varyings vert(Attributes input)
            {
                ...
                output.emissionUV = TRANSFORM_TEX(input.uv, _EmissionMask);
            ...
            float3 GetStylizedLight(float3 positionWS, float3 normalWS, float4 shadowCoord, out float lightAmount)
            {
                ...
                return max(lightColor, float3(_Ambient, _Ambient, _Ambient));
            }
            ...
            half4 frag(Varyings input) : SV_Target
            {
                ...
                float emissionMask = saturate(SAMPLE_TEXTURE2D(_EmissionMask, sampler_EmissionMask, input.emissionUV).r);
                baseColor = lerp(baseColor, _PaintColor.rgb, emissionMask * _PaintStrength);
